...
import { useCoinbaseTradesAutomation } from "@/hooks/useCoinbaseTradesAutomation";
...
export function CoinbaseTradesFlow() {
  const navigate = useNavigate();
  const [isCheckingAuth, setIsCheckingAuth] = useState(true);

  const { isConnected, checkStatus } = useComposio('COINBASE');
  ...
  useEffect(() => {
    if (!isCheckingAuth && isConnected) {
      ...
      sessionStorage.setItem('returnAfterCoinbaseConnect', '/flow/coinbase-trades');
      navigate('/integration/coinbase');
    }
      ...
      <div className={cn("relative px-5 pt-status-bar pb-6", gradientClasses.blue)}>
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            <h1 className="text-xl font-bold text-white truncate">Coinbase Trades to Memory</h1>
