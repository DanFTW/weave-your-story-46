...
import { useFirefliesAutomation } from "@/hooks/useFirefliesAutomation";
...
export function FirefliesAutomationFlow() {
  ...
  useEffect(() => {
    if (!isCheckingAuth && isConnected) {
      ...
      sessionStorage.setItem('returnAfterFirefliesConnect', '/flow/fireflies-tracker');
      navigate('/integration/fireflies');
    }
  ...
  const getSubtitle = () => {
    switch (phase) {
      ...
      default: return 'Transcript tracker';
    }
  };
      ...
      <div className={cn("relative px-5 pt-status-bar pb-6", gradientClasses.purple)}>
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            <h1 className="text-xl font-bold text-white truncate">Fireflies Transcript Tracker</h1>
