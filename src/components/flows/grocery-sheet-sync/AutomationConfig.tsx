...
import { ShoppingCart, ArrowRight, Plus, Loader2, ChevronDown } from "lucide-react";
import { GrocerySheetSyncConfig, SpreadsheetOption } from "@/types/grocerySheetSync";

interface AutomationConfigProps {
  config: GrocerySheetSyncConfig;
      ...
      <div className="bg-card rounded-2xl border border-border p-5 space-y-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-muted-foreground text-sm">Memories → Google Sheet rows</p>
          </div>
        </div>

        <div className="space-y-3">
          {[
            "When you save a memory, we parse it for grocery items",
            "Items found get appended as rows to your selected Google Sheet",
            "Use Sync Now to scan existing memories for grocery items",
          ].map((text, i) => (
