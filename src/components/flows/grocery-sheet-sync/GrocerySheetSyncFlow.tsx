...
import { useGrocerySheetSync } from "@/hooks/useGrocerySheetSync";
...
export function GrocerySheetSyncFlow() {
  const navigate = useNavigate();
  const [isCheckingAuth, setIsCheckingAuth] = useState(true);

  const { isConnected, checkStatus } = useComposio("GOOGLESHEETS");
  ...
  useEffect(() => {
    if (!isCheckingAuth && isConnected) {
      ...
      sessionStorage.setItem("returnAfterGooglesheetsConnect", "/flow/grocery-sheet-sync");
      ...
      <div className={cn("relative px-5 pt-status-bar pb-6", gradientClasses.teal)}>
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            <h1 className="text-xl font-bold text-white truncate">Grocery Memory to Google Sheet</h1>
