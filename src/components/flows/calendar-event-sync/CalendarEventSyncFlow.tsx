...
import { useCalendarEventSync } from "@/hooks/useCalendarEventSync";
...
export function CalendarEventSyncFlow() {
  const navigate = useNavigate();
  const [isCheckingAuth, setIsCheckingAuth] = useState(true);

  const { isConnected, checkStatus } = useComposio("GOOGLECALENDAR");
  ...
  useEffect(() => {
    if (!isCheckingAuth && isConnected) {
      ...
      sessionStorage.setItem("returnAfterGooglecalendarConnect", "/flow/calendar-event-sync");
      navigate("/integration/googlecalendar");
    }
      ...
      <div className={cn("relative px-5 pt-status-bar pb-6", gradientClasses.blue)}>
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            <h1 className="text-xl font-bold text-white truncate">Event Memory to Calendar</h1>
      ...
      <div className="px-5 pt-6">
        ...
        {phase === "active" && (
          <ActiveMonitoring
            ...
            onPause={deactivate}
            onUpdateEvent={updatePendingEvent}
