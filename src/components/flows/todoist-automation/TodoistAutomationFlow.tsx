...
import { useTodoistAutomation } from "@/hooks/useTodoistAutomation";
...
export function TodoistAutomationFlow() {
  ...
  useEffect(() => {
    if (!isCheckingAuth && isConnected) {
      ...
      sessionStorage.setItem('returnAfterTodoistConnect', '/flow/todoist-task-tracker');
      navigate('/integration/todoist');
    }
  ...
  if (isCheckingAuth || isLoading) {
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <Loader2 className="w-8 h-8 text-[#E44332] animate-spin" />
      </div>
    );
  }
      ...
      <div className={cn("relative px-5 pt-status-bar pb-6", gradientClasses.orange)}>
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            <h1 className="text-xl font-bold text-white truncate">Todoist Task Tracker</h1>
