...
import { Image, MessageCircle, Film, Loader2, ArrowRight } from "lucide-react";
...
interface InstagramSyncConfigProps {
  ...
  syncStories: boolean;
  isSaving: boolean;
  onSave: (config: { syncPosts: boolean; syncComments: boolean; syncStories: boolean }) => Promise<void>;
  onStartSync: () => void;
}
...
export function InstagramSyncConfig({
  ...
  syncStories: initialSyncStories,
  isSaving,
  onSave,
  onStartSync,
  ...
  const [syncStories, setSyncStories] = useState(initialSyncStories);

  const handleSaveAndSync = async () => {
    await onSave({ syncPosts, syncComments, syncStories });
    onStartSync();
  };
    ...
    <div className="space-y-6">
      <div className="bg-muted/50 rounded-xl p-4">
        <p className="text-sm text-muted-foreground">
          Choose what content to sync from your Instagram account. Your posts, stories, and comments 
      ...
      <div className="space-y-4">
        ...
        <div className="flex items-center justify-between p-4 bg-card rounded-xl border border-border">
          ...
          <Switch checked={syncPosts} onCheckedChange={setSyncPosts} />
        </div>
        ...
        <div className="flex items-center justify-between p-4 bg-card rounded-xl border border-border">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-medium">Sync Stories</p>
              <p className="text-sm text-muted-foreground">Capture stories before they disappear</p>
            </div>
          </div>
          <Switch checked={syncStories} onCheckedChange={setSyncStories} />
        </div>
      </div>
      ...
      <Button
        ...
        disabled={isSaving || (!syncPosts && !syncComments && !syncStories)}
