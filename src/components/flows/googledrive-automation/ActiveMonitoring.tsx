import { FileText, Pause, RefreshCw } from "lucide-react";
import { Button } from "@/components/ui/button";
import { GoogleDriveAutomationStats } from "@/types/googledriveAutomation";
import { formatDistanceToNow } from "date-fns";

interface ActiveMonitoringProps {
  stats: GoogleDriveAutomationStats;
  onPause: () => void;
  onCheckNow: () => void;
  isSyncing?: boolean;
}
    ...
    <div className="space-y-6">
      ...
      <div className="bg-card rounded-xl p-5 border border-border">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              {stats.lastSyncAt
                ? `Last checked ${formatDistanceToNow(new Date(stats.lastSyncAt), { addSuffix: true })}`
        ...
        <div className="grid grid-cols-1 gap-4 mt-4">
          <div className="bg-muted/50 rounded-lg p-3 text-center">
            <div className="text-2xl font-bold text-foreground">{stats.documentsSaved}</div>
            <div className="text-xs text-muted-foreground mt-1">Documents Saved</div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-card rounded-xl border border-border p-4">
        ...
        <div className="flex items-center gap-3 text-sm">
          ...
          <span className="text-foreground">New Google Docs</span>
