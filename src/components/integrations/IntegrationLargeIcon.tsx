...
// Large brand icons for detail page
const largeBrandIcons: Record<string, React.ReactNode> = {
  gmail: (
  ...
  instagram: (
    <svg viewBox="0 0 24 24" className="w-12 h-12">
      <defs>
        <linearGradient id="instagram-gradient-lg" x1="0%" y1="100%" x2="100%" y2="0%">
      ...
      <rect x="2" y="2" width="20" height="20" rx="5" fill="url(#instagram-gradient-lg)"/>
...
export function IntegrationLargeIcon({ icon, className }: IntegrationLargeIconProps) {
  ...
  if (largeBrandIcons[icon]) {
    return (
      <div className={cn(
        "w-20 h-20 rounded-2xl overflow-hidden flex items-center justify-center bg-white shadow-xl",
        className
      )}>
        {largeBrandIcons[icon]}
      </div>
    );
  }
  ...
  return (
    <div className={cn(
      "w-20 h-20 rounded-2xl bg-muted flex items-center justify-center shadow-xl",
      className
    )}>
