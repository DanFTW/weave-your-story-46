...
import {
  GrocerySheetSyncPhase,
  GrocerySheetSyncConfig,
  GrocerySheetSyncStats,
  SpreadsheetOption,
} from "@/types/grocerySheetSync";

export function useGrocerySheetSync() {
  const { toast } = useToast();
  const [phase, setPhase] = useState<GrocerySheetSyncPhase>("auth-check");
  const [config, setConfig] = useState<GrocerySheetSyncConfig | null>(null);
  ...
  const stats: GrocerySheetSyncStats = {
    itemsPosted: config?.itemsPosted ?? 0,
  ...
  const loadConfig = useCallback(async () => {
    ...
    try {
      ...
      if (error) {
        console.error("Error loading grocery sheet config:", error);
        return;
      }
  ...
  const listSpreadsheets = useCallback(async () => {
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke("grocery-sheet-sync", {
  ...
  const updateConfig = useCallback(async (spreadsheetId: string, spreadsheetName: string) => {
    try {
      ...
      await supabase.functions.invoke("grocery-sheet-sync", {
  ...
  const activate = useCallback(async (): Promise<boolean> => {
    ...
    try {
      ...
      const { error } = await supabase.functions.invoke("grocery-sheet-sync", {
      ...
      setPhase("active");
      toast({ title: "Grocery sync activated", description: "New grocery memories will be posted to your sheet" });
      return true;
    } catch {
  ...
  const deactivate = useCallback(async (): Promise<boolean> => {
    try {
      ...
      setPhase("configure");
      toast({ title: "Grocery sheet sync paused" });
      return true;
    } catch {
      return false;
    }
  ...
  const manualSync = useCallback(async () => {
    ...
    try {
      ...
      toast({
        title: "Sync complete",
        description: `Processed ${result.processed ?? 0} memories — ${result.posted ?? 0} grocery items posted`,
      });
