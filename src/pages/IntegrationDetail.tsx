...
export default function IntegrationDetail() {
  ...
  const handleChangeAccount = async () => {
    // Check if this provider supports programmatic account switching
    if (integration?.supportsAccountSwitch === false) {
      // Show guidance dialog for manual logout
      setShowAccountSwitchDialog(true);
    } else {
      // Standard flow: disconnect and reconnect with force_reauth
      await disconnect();
  ...
  const handleAccountSwitchConfirm = async () => {
    // Proceed with disconnect + reconnect after user confirms manual logout
    await disconnect();
    await connect(`/integration/${integrationId}`, true);
  };
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      {/* Account Switch Dialog for providers that don't support forceReauth */}
      <AccountSwitchDialog
